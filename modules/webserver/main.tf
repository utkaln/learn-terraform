...
resource "aws_instance" "ansible-EC2-one" {
  ...
  tags = {
    Name = "${var.env_prefix}-server-instance-one"
  }
}

resource "aws_instance" "ansible-EC2-two" {
  // Use the ami id that matches the desired description
  ami = data.aws_ami.utkal-ami.id
  ...
  tags = {
    Name = "${var.env_prefix}-server-instance-two"
  }
}

resource "aws_instance" "ansible-EC2-three" {
  // Use the ami id that matches the desired description
  ami = data.aws_ami.utkal-ami.id
  ...
  tags = {
    Name = "${var.env_prefix}-server-instance-three"
  }
}
